...
Bitboard set_occupancy(int index, int number_of_attacks, Bitboard attack_table){
    ...
    for (int count = 0; count < number_of_attacks; count++){
        int square = get_lsb_index(attack_table);
        
        pop_bit(attack_table, square);
...
Bitboard find_magic_number(int square, int relevant_bits, int bishop){
    ...
    Bitboard attack_mask = bishop ? generate_bishop_attacks(square) : generate_rook_attacks(square);
    ...
    for (int index = 0; index < occupancy_indicies; index++){
        ...
        attacks[index] = bishop ? bishop_attacks_innit(square, occupancies[index]) :
                                    rook_attacks_innit(square, occupancies[index]);
    }
    ...
    for (int square = 0;square < 64; square++){
        ...
        for(int index = 0; index < occupancy_indices; index++){
            ...
            if (bishop) {
                ...
                bishop_attacks[square][magic_index] = bishop_attacks_innit(square, occupancy);

            } else {
                Bitboard occupancy = set_occupancy(index, relevant_bit_count, attack_mask);
                int magic_index = (occupancy * rook_magic_numbers[square]) >> (64 - rook_relevant_bits[square]);
                rook_attacks[square][magic_index] = rook_attacks_innit(square, occupancy);
            }
