...
std::string get_move_as_string(int move) {
    ...
    int source_square = move & 0x3f;
    int target_square = (move >> 6) & 0x3f;
    int promoted_piece = (move >> 16) & 0xf;

    std::string move_str = "";
    move_str += square_to_coordinates[source_square];
    move_str += square_to_coordinates[target_square];

    if (promoted_piece) {
        move_str += promoted_pieces[promoted_piece];
    }
